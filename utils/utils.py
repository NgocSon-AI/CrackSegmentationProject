...
config = yaml.load(open('./configs/config_crack.yml'), Loader=yaml.FullLoader)
...
class DiceBCELoss(nn.Module):
    def __init__(self, smooth=1.0, pos_weight=1.0, from_logits=False):
        super(DiceBCELoss, self).__init__()
        self.smooth = smooth
        self.from_logits = from_logits
        self.bce = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(pos_weight)) if from_logits else nn.BCELoss()

    def forward(self, inputs, targets):
        """
        inputs: predicted mask (sigmoid already applied if from_logits=False)
        ...
        if self.from_logits:
            ...
            # If sigmoid already applied outside
            bce_loss = self.bce(inputs, targets)
            probs = inputs
